...
labels[future_return > 0.0025] = 1  # LONG
labels[future_return < -0.0025] = 2  # SHORT
...
close_prices = df[f"ema_20_{INTERVALS[0]}"].values[valid_idx][window:]
...
class_weights_array = compute_class_weight(class_weight="balanced", classes=classes, y=y_train_raw)
...
def build_model_gru():
    model = Sequential([
        ...
        Dropout(0.4),
        GRU(32, kernel_regularizer=l2(L2_REG)),
...
def build_model_lstm_attention():
    ...
    x = LSTM(64, return_sequences=True, kernel_regularizer=l2(L2_REG))(inputs)
    x = Dropout(0.4)(x)
...
def build_cnn_lstm_model():
    model = Sequential([
        Input(shape=(X_train.shape[1], X_train.shape[2])),
        Conv1D(filters=64, kernel_size=3, activation='relu', kernel_regularizer=l2(L2_REG)),
...
def build_transformer_encoder_model():
    ...
    dense_output = Dense(inputs.shape[-1], activation='relu', kernel_regularizer=l2(L2_REG))(x)
...
if __name__ == "__main__":
    ...
    for name, model in models.items():
        ...
        checkpoint = ModelCheckpoint(
            BEST_MODEL_FILE,  # dùng .h5 thay vì .keras để tránh lỗi options
    ...
    min_loss = min(min(r["history"].history["val_loss"]) for r in results)
    max_loss = max(min(r["history"].history["val_loss"]) for r in results)

    def normalize_profit(p):
        return (p - min_profit) / (max_profit - min_profit) if max_profit > min_profit else 0.0

    def normalize_loss(loss):
        return (max_loss - loss) / (max_loss - min_loss) if max_loss > min_loss else 0.0
    ...
    for r in results:
        ...
        norm_loss = normalize_loss(best_val_loss)

        r["combined_score"] = (
                r["val_acc"] * acc_weight +
                norm_profit * profit_weight +
                norm_loss * loss_weight
        )
